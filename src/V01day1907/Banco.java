...
public class Banco {
    ...
    public static ContaPoupanca acessaCP(ContaPoupanca c1) {
        ...
        do {
            ...
            switch (opc) {
                ...
                case "v" -> System.out.println(c1.verificaSaldo1());
                case "a" -> {
                    String senha = leString("Qual a nova senha");
                    c1.setSenha(senha);
                }
    ...
    public static ContaPoupanca cadastraCP() {
        ...
        System.out.println("Conta cadastrada com o identificador " + c1.getIdentificador1());
        return c1;
    }

    public static void gravaContas(ContaCorrente[] listaCC, ContaPoupanca[] listaCP, int contCC, int contCP) {
        try {
            ObjectOutputStream objEscritaCC = new ObjectOutputStream(new FileOutputStream("contasCorrentes.db"));
            for (int i = 0; i < contCC; i++) {
                objEscritaCC.writeObject(listaCC[i]);
            }
            objEscritaCC.close();

            ObjectOutputStream objEscritaCP = new ObjectOutputStream(new FileOutputStream("contasPoupancas.db"));
            for (int i = 0; i < contCP; i++) {
                objEscritaCP.writeObject(listaCP[i]);
            }
            objEscritaCP.close();
        } catch (IOException e) {
            System.out.println("Erro ao gravar as contas: " + e.getMessage());
        }
    }

    public static ContaCorrente[] carregaContasCorrentes() {
        try {
            ObjectInputStream objLeituraCC = new ObjectInputStream(new FileInputStream("contasCorrentes.db"));
            ContaCorrente[] listaCC = new ContaCorrente[20];
            int i = 0;
            while (true) {
                listaCC[i] = (ContaCorrente) objLeituraCC.readObject();
                i++;
            }
        } catch (EOFException e) {
            
        } catch (IOException | ClassNotFoundException e) {
            System.out.println("Erro ao carregar as contas correntes: " + e.getMessage());
        }
        return new ContaCorrente[20]; 
    }

    public static ContaPoupanca[] carregaContasPoupancas() {
        try {
            ObjectInputStream objLeituraCP = new ObjectInputStream(new FileInputStream("contasPoupancas.db"));
            ContaPoupanca[] listaCP = new ContaPoupanca[20];
            int i = 0;
            while (true) {
                listaCP[i] = (ContaPoupanca) objLeituraCP.readObject();
                i++;
            }
        } catch (EOFException e) {
           
        } catch (IOException | ClassNotFoundException e) {
            System.out.println("Erro ao carregar as contas poupanças: " + e.getMessage());
        }
        return new ContaPoupanca[20]; 
    }

    public static void main(String[] args) {
        ContaCorrente[] listaCC = carregaContasCorrentes();
        ContaPoupanca[] listaCP = carregaContasPoupancas();
        int contCC = 0;
        int contCP = 0;
        String opc;
        do {
        ...
        gravaContas(listaCC, listaCP, contCC, contCP);
    }
}
